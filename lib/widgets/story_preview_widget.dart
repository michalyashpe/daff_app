...
Widget buildStoryPreviewWidget(Story story, BuildContext context,
    {bool tagView = false, bool authorName = true}) {
  return Container(
      ...
      child: GestureDetector(
          onTap: () {
            Navigator.push(context,
                MaterialPageRoute(builder: (context) => StoryScreen(story.id)));
          },
          child: IntrinsicHeight(
              child: Row(
            children: <Widget>[
              Expanded(flex: 2, child: _buildStoryImage(story, context)),
              SizedBox(
                width: 10.0,
              ),
              Expanded(
                  ...
                  child: _buildStoryInfo(story, context,
                      tagView: tagView, authorName: authorName))
            ],
          ))));
}
  ...
  return Container(
    ...
    children: <Widget>[
      Container(
        ...
        WidgetSpan(
          child: buildEditerPickMedalWidget(story),
        ),
      ]))),
      Text(
        authorName
            ? '${story.readingDurationString} \u{00B7} ${story.author.name}'
...
Widget buildStoryPreviewLoaderWidget(BuildContext context) {
  return Container(
      ...
      child: IntrinsicHeight(
          child: Row(
        children: <Widget>[
          Expanded(flex: 2, child: buildShimmeringBox(height: 100.0)),
          Expanded(
              flex: 3,
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
