...
class HtmlHelper {
  static Widget buildHtml(Story story, BuildContext context) {
    return Html(
      ...
      customTextStyle: (dom.Node node, TextStyle baseStyle) {
        ...
        return baseStyle.merge(GoogleFonts.alef()).merge(TextStyle(
              fontSize: fontSize,
              height: height,
              color: color,
              decoration: textDecoration,
            )); // for hr repleace with a widget
      },
  ...
  static String replaceHrWithImage(String html) {
    ...
    document.getElementsByTagName('hr').forEach((tag) {
      String html =
          '<figure class="image"><img src="$img" width="70"></figure>';
  ...
  static void linkTapHandler(String url, BuildContext context) {
    if (!url.contains('daff.co.il')) {
      ...
      String cleanUrl =
          url.replaceAll('https://', '').replaceAll('daff.co.il', '');
      if (cleanUrl == '')
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => StoriesScreen('בית', 'hits=true')));
      else if (cleanUrl == '/stories')
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => StoriesScreen('כל הסיפורים', '')));
      else if (cleanUrl == '/users/sign_up')
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => ConnectScreen()));
      else if (cleanUrl.contains(('with_audio=true')))
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) =>
                StoriesScreen('דפים מוקלטים', 'with_audio=true')));
      else if (cleanUrl.contains('editor_votes=true'))
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) =>
                StoriesScreen('בחירות העורך', 'editor_votes=true')));
        ...
        if (match != null) {
          ...
          Navigator.push(
              context,
              MaterialPageRoute(
                  builder: (context) => StoriesScreen(tag, 'tag=$tag')));
        }
      } else {
        //link to story
        ...
        if (match != null) {
          ...
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => StoryScreen(storyId)));
        } else {
