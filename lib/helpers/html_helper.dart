  ...
  static void linkTapHandler(String url, BuildContext context){
    if(!url.contains('daff.co.il')){
      ...
      if (cleanUrl =='')  Navigator.of(context).push(MaterialPageRoute(builder: (context) => StoriesScreen('בית', 'hits=true')));
        ...
        Navigator.of(context).push(MaterialPageRoute(builder: (context) => StoriesScreen('דפים מוקלטים', 'with_audio=true')));
      else if (cleanUrl.contains('editor_votes=true')) 
        Navigator.of(context).push(MaterialPageRoute(builder: (context) => StoriesScreen('בחירות העורך', 'editor_votes=true')));
