...
class AuthModel extends ChangeNotifier {
  ...
  Future<bool> tryAutoLogin() async {
    ...
    if (!prefs.containsKey('authenticationToken') ||
        prefs.get('daffServerUrl') != daffServerUrl) {
...
// Email LOGIN ---------------------
  Future<int> emailLogin() async {
    ...
    http.Response response = await http
        .post(daffServerUrl + '/users/sign_in.json', headers: <String, String>{
  ...
  Future<int> emailSignUp() async {
    ...
    isLoading = true;
    notifyListeners();
    await http.post(daffServerUrl + '/users.json', headers: <String, String>{
      'authorization': basicAuth,
    }, body: {
      'user[doar]': email,
      'user[password]': password
      ...
      if (result['errors'] == 'inactive_sign_up') {
        ...
        newAccount = false;
        errors.add(
            'אנחנו כמעט שם. יש לאשר את המייל שנשלח אליך ולאחר מכן להתחבר .');
        notifyListeners();
      } else if (result['errors']['password'] != null)
        result['errors']['password']
            .forEach((e) => errors.add('סיסמה לא תקינה: $e'));
      else if (result['errors']['email'] != null)
        result['errors']['email']
            .forEach((e) => errors.add(' כתובת מייל לא תקינה: $e'));
      if (status == 200) {
